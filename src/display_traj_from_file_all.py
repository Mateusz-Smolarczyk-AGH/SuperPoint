...
file_results_float = args.maindir / "results/tum_freiburg1_floor_float.txt"
file_results_8bits = args.maindir / "results/tum_freiburg1_floor_8bit.txt"
file_results_4bits = args.maindir / "results/tum_freiburg1_floor_4bit.txt"
file_results_3bits = args.maindir / "results/tum_freiburg1_floor_3bit.txt"
file_results_424bits = args.maindir / "results/tum_freiburg1_floor_4_2_4bit.txt"
...
data_float = np.loadtxt(file_results_float, delimiter=" ")
data_8bits = np.loadtxt(file_results_8bits, delimiter=" ")
data_4bits = np.loadtxt(file_results_4bits, delimiter=" ")
data_3bits = np.loadtxt(file_results_3bits, delimiter=" ")
data_424bits = np.loadtxt(file_results_424bits, delimiter=" ")
# print(data.shape)

est_timestamp_float = data_float[:, 0]
est_trajectory_float = data_float[:, 1:4]
est_quat_float = data_float[:, 4:8]

est_rot_float = R.from_quat(est_quat_float)  # scipy używa kolejności: x, y, z, w
est_euler_float = est_rot_float.as_euler("zyx", degrees=True)  # yaw, pitch, roll

est_timestamp_8bits = data_8bits[:, 0]
est_trajectory_8bits = data_8bits[:, 1:4]
est_quat_8bits = data_8bits[:, 4:8]
est_rot_8bits = R.from_quat(est_quat_8bits)  # scipy używa kolejności: x, y, z, w
est_euler_8bits = est_rot_8bits.as_euler("zyx", degrees=True)  # yaw, pitch, roll

est_timestamp_4bits = data_4bits[:, 0]
est_trajectory_4bits = data_4bits[:, 1:4]
est_quat_4bits = data_4bits[:, 4:8]
est_rot_4bits = R.from_quat(est_quat_4bits)  # scipy używa kolejności: x, y, z, w
est_euler_4bits = est_rot_4bits.as_euler("zyx", degrees=True)  # yaw, pitch, roll

est_timestamp_3bits = data_3bits[:, 0]
est_trajectory_3bits = data_3bits[:, 1:4]
est_quat_3bits = data_3bits[:, 4:8]
est_rot_3bits = R.from_quat(est_quat_3bits)  # scipy używa kolejności: x, y, z, w
est_euler_3bits = est_rot_3bits.as_euler("zyx", degrees=True)  # yaw, pitch, roll

est_timestamp_424bits = data_424bits[:, 0]
est_trajectory_424bits = data_424bits[:, 1:4]
est_quat_424bits = data_424bits[:, 4:8]
est_rot_424bits = R.from_quat(est_quat_424bits)  # scipy używa kolejności: x, y, z, w
est_euler_424bits = est_rot_424bits.as_euler("zyx", degrees=True)  # yaw, pitch, roll
...
for i in range(3):
    plt.subplot(3, 1, i + 1)
    plt.plot(est_euler_float[:, i], label="Float")
    plt.plot(est_euler_8bits[:, i], label="8 bits")
    plt.plot(est_euler_4bits[:, i], label="4 bits")
    plt.plot(est_euler_3bits[:, i], label="3 bits")
    plt.plot(est_euler_424bits[:, i], label="4-2-4 bits")
    plt.plot(gt_euler[:, i], label="GT", linestyle="--")
    plt.legend()
    plt.ylabel(labels[i])
