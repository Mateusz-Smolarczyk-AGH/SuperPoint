...
class Feature_detection:
    def __init__(self, image_size, weight_path=None):
...
class VisualOdometry:
    def __init__(
        self, image_size, start_R, start_t, cam: PinholeCamera, weight_path=None
    ):
        self.keypoints = {"past": None, "present": None}

        self.descriptors = {"past": None, "present": None}
        self.matches = None
        self.feature_detection = Feature_detection(image_size, weight_path)
    ...
    def compute_first_image(self, image):
        ...
        self.keypoints["past"], self.descriptors["past"], _, _ = (
    ...
    def compute_pipeline(self, image):
        ...
        self.keypoints["present"], self.descriptors["present"], net, post = (
            self.feature_detection.process_Superpoint()
        )
        ...
        pts1 = np.float32(
            [self.keypoints["past"][match.queryIdx].pt for match in self.matches]
        )  # shape (N, 2)
        pts2 = np.float32(
            [self.keypoints["present"][match.trainIdx].pt for match in self.matches]
        )  # shape (N, 2)
